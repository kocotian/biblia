#!/bin/sh
# biblia: Read the Word of God in Greek from your terminal
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data biblia.tsv | awk -v cmd=list "$(get_data biblia.awk)"
		exit
	elif [ "$1" = "-W" ]; then
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data biblia.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data biblia.awk)" | ${PAGER}
	done
	exit 0
fi

get_data biblia.tsv 2>/dev/null | awk -v cmd=ref -v ref="$*" "$(get_data biblia.awk)" 2>/dev/null | ${PAGER}
